...
def exploded_numbers(ints, n):
    ...
    list_len = len(ints)
    max_len = len(str(ints[list_len - 1] + n))

    output = []
    for i in ints:
        nums = [str(x).zfill(max_len) for x in range(i - n, i + n + 1)]
...
def arr_1(A):
    ...
    return np.add(A, np.sqrt(indices))
...
def arr_3(A):
    ...
    numerator = np.delete(A, 0)
    denominator = np.delete(A, len(A) - 1)

    return np.round(np.divide(numerator, denominator) - 1, 2)
...
def arr_4(A):
    ...
    remainders = start_money % A
    left_over = np.cumsum(remainders)

    return np.where(left_over >= A)[0][0]
...
def salary_stats(salary):
    ...
    output.append(len(salary))
    output.append(salary['Team'].nunique())
    output.append(salary['Salary'].sum())
    output.append(salary.iloc[salary['Salary'].argmax()][0])
    output.append(salary.loc[salary['Team'] == 'BOS']['Salary'].mean())

    third_lowest = salary.loc[np.argsort(salary['Salary'])[3]]
    output.append(third_lowest['Player'] + ', ' + third_lowest['Team'])
    output.append(output[0] == len(np.unique(salary['Player'].map(lambda x: x.split(' ')[-1]))))
    output.append(
        salary.loc[salary['Team'] == salary.loc[salary['Player'] == output[3]]['Team'].values[0]]['Salary'].sum())

    return pd.Series(output, index=['num_players', 'num_teams', 'total_salary', 'highest_salary', 'avg_bos',
                                    'third_lowest', 'duplicates', 'total_highest'])
...
def parse_malformed(fp):
    ...
    with open(fp, 'r') as f:
        ...
        for line in lines:
            ...
            clean_input[0].append(inputs[0].strip('\"'))
            clean_input[1].append(inputs[1].strip('\"'))
            clean_input[2].append(float(inputs[2].strip('\"')))
            clean_input[3].append(float(inputs[3].strip('\"')))
            clean_input[4].append(inputs[4].strip('\"') + ',' + inputs[5].strip('\"'))

    return pd.DataFrame({names[0]: clean_input[0], names[1]: clean_input[1], names[2]: clean_input[2],
                         names[3]: clean_input[3], names[4]: clean_input[4]})
